...
DEFAULT_YOLO_WEIGHTS = Path(os.environ.get("YOLO_WEIGHTS", REPO_ROOT / "weights" / "esophagus_best.pt"))

VIDEO_DIR = Path(os.environ.get("VIDEO_DIR", REPO_ROOT / "data" / "videos" / "esophagus"))
OUTPUT_DIR = Path(os.environ.get("OUTPUT_DIR", REPO_ROOT / "outputs" / "detect_kalman" / "esophagus"))
VIDEO_EXTS = [".avi", ".mp4"]
...
def resolve_weights_path(path: Path) -> Path:
    ...
    if run_dir.exists():
        candidates = sorted(run_dir.parent.glob(f"{run_dir.name}*"), key=lambda p: p.stat().st_mtime, reverse=True)
        for candidate in candidates:
            candidate_file = candidate / "weights" / path.name
            if candidate_file.exists():
                return candidate_file
    raise FileNotFoundError(f"YOLO weights not found at {path}. Please check your training run directory.")

YOLO_WEIGHTS = resolve_weights_path(DEFAULT_YOLO_WEIGHTS)
...
CLASS_NAMES = ['inlet', 'lumen', 'stenosis']
...
def main():
    ...
    videos = sorted([p for ext in VIDEO_EXTS for p in video_dir.glob(f"*{ext}")])
